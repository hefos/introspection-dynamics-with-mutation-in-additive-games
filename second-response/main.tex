...
\pending{Reorder the abstract and introduction so that the extension of
introspection dynamics to mutation is stated first, with the additive-game
results then rederived from it.}
...
\pending{Expand the introduction's related work: logit-response dynamics
(Blume; Al\'os-Ferrer and Netzer), evolutionary game theoretic models of
asymmetric games, and mutation and exploration in learning dynamics.}
...
\pending{Restructure the proof of Theorem 1 along exactly these lines: define
the per-player state space \(\{C,D\}\) and the per-player transition matrix
with identical rows \((p_i,\,1-p_i)\), from which the stationary marginal is
immediate. This same two-state-chain framework also supports the alternative
mutation scheme raised by Reviewer 2.}
...
\pending{Add a numerical study of a non-linear public goods game with
mutation, computing the stationary distribution from the full \(2^N\) linear
system, and report whether mutation can raise cooperation where selection
alone would not.}
...
\pending{Attempt the extension to \(L\) strategies with mutation, at least in
the symmetric-mutation case; fold the result into the restructured Theorem 1
if it goes through.}
...
\(M_3\), and the acronym PD is defined at first use.
...
We now write `components' rather than `quantities'. \pending{Rename the
mutation probabilities so that the subscripts agree with the action coding
(mutation to \(C\) and to \(D\)); this notation change ripples through every
statement and figure, so we will carry it out in a single dedicated pass.}
...
The wording now refers to selections `resulting in action \(C\)' rather than
placing a player `in state \(C\)'. 
...
\pending{Rewrite the conclusion so that, together with the introduction, it
is self-contained.}
