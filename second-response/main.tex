...
\(N\) systems of dimension \(L_i\), and the marginals admit the explicit
representation \(m_i=\mu_i(I-(1-M_i)Q_i)^{-1}\), with \(Q_i\) the
mutation-free kernel and \(M_i=\sum_b\mu_{ib}\). We also record what is
special to two actions: the identical-rows property behind the closed form
holds only there, and with mutation the marginal is in general no longer of
Boltzmann form; without mutation it is, recovering Proposition 2 of [2].
