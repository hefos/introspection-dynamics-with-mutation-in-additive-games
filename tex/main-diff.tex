...
%DIF DEL /tmp/main_old.tex   Mon Aug 10 10:44:49 2026
%DIF ADD main.tex            Mon Aug 10 10:44:41 2026
...
At each step, one player \(i\) is selected uniformly at random \DIFdelbegin \DIFdel{, and draws }\DIFdelend \DIFaddbegin \DIFadd{and updates
...
the remaining probability \(1-M_i\), they introspect: they draw }\DIFaddend an
alternative action \(b\) uniformly from the \(L_i-1\) actions other than
\(a_i\) \DIFaddbegin \DIFadd{and switch to it with probability
\(\phi_i\bigl(\Delta f_i(\bfa;b)\bigr)\)}\DIFaddend . The transition probability \DIFdelbegin \DIFdel{for the switch is }\DIFdelend \DIFaddbegin \DIFadd{to
each state \(\bfa_{i\to b}\) with \(b\neq a_i\) is therefore
}\begin{equation}\DIFadd{\label{eq:intro-transition-general}
  T^{\intro}_{\bfa,\,\bfa_{i\to b}}
  = \frac{1}{N}\left[(1-M_i)\,
    \frac{\phi_i\bigl(\Delta f_i(\bfa;b)\bigr)}{L_i-1}+\mu_{ib}\right],
  \qquad b\neq a_i,
}\end{equation}
\DIFadd{and the state is unchanged with the remaining probability,
}\DIFaddend \begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq:intro-transition-general}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \label{eq:holding}
  \DIFaddend T^{\intro}\DIFdelbegin \DIFdel{_{\bfa,\,\bfa_{i\to b}}
  }\DIFdelend \DIFaddbegin \DIFadd{_{\bfa,\,\bfa}
  }\DIFaddend = \DIFdelbegin \DIFdel{\frac{1}{N}}%DIFDELCMD < \left[%%%
\DIFdel{(1-M_i)\,
    \frac{\phi_i\bigl(\Delta f_i(\bfa;b)\bigr)}{L_i-1}+\mu_{ib}}%DIFDELCMD < \right]%%%
\DIFdelend \DIFaddbegin \DIFadd{1-\sum_{i=1}^{N}\sum_{b\neq a_i} T^{\intro}_{\bfa,\,\bfa_{i\to b}}}\DIFaddend ,
\DIFdelbegin \DIFdel{\qquad b\neq a_i.
}\DIFdelend \end{equation}
\DIFaddbegin \DIFadd{which collects the events that the selected player mutates to their
current action or introspects without switching.
}\DIFaddend Since \(\phi_i\) is strictly decreasing, a larger payoff gain from keeping
...
\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{eq:kernel}
  K_i(a_i,b)
  = (1-M_i)\,\frac{\phi_i\bigl(g_i(a_i)-g_i(b)\bigr)}{L_i-1}+\mu_{ib},
  \qquad b\neq a_i,
...
unique, strictly positive stationary distribution \DIFdelbegin \DIFdel{\(m_i\) }\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i\) }\DIFaddend on \(A_i\).
...
\begin{equation}\label{eq:stationary-general}
  \pi_{\bfa} = \prod_{i=1}^{N} \DIFdelbegin \DIFdel{m}\DIFdelend \DIFaddbegin \DIFadd{\bfm}\DIFaddend _i(a_i), \qquad \bfa\in S,
\end{equation}
where \DIFdelbegin \DIFdel{\(m_i\) }\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i\) }\DIFaddend is the unique stationary distribution of \(K_i\).
...
\]
The \DIFaddbegin \DIFadd{first expression follows from the holding
probability~\eqref{eq:holding} and the diagonal entries
\(K_i(a_i,a_i)=1-\sum_{b\neq a_i}K_i(a_i,b)\).
The }\DIFaddend product form gives
\DIFdelbegin \DIFdel{\(\pi_{\bfa_{i\to b}}\,m_i(a_i)=\pi_{\bfa}\,m_i(b)\), so
}\DIFdelend \DIFaddbegin \DIFadd{\(\pi_{\bfa_{i\to b}}\,\bfm_i(a_i)=\pi_{\bfa}\,\bfm_i(b)\), so
}\DIFaddend \[
  \bigl(\pi T^{\intro}\bigr)_{\bfa}
  = \frac{1}{N}\sum_{i=1}^{N}
    \Bigl[\pi_{\bfa}K_i(a_i,a_i)
      +\sum_{b\neq a_i}\pi_{\bfa_{i\to b}}K_i(b,a_i)\Bigr]
  = \frac{1}{N}\sum_{i=1}^{N}\DIFdelbegin \DIFdel{\frac{\pi_{\bfa}}{m_i(a_i)}
    }\DIFdelend \DIFaddbegin \DIFadd{\frac{\pi_{\bfa}}{\bfm_i(a_i)}
    }\DIFaddend \sum_{b\in A_i} \DIFdelbegin \DIFdel{m}\DIFdelend \DIFaddbegin \DIFadd{\bfm}\DIFaddend _i(b)\,K_i(b,a_i)
  = \pi_{\bfa},
\]
since \DIFdelbegin \DIFdel{\(\sum_{b} m_i(b)K_i(b,a_i)=m_i(a_i)\) }\DIFdelend \DIFaddbegin \DIFadd{\(\sum_{b} \bfm_i(b)K_i(b,a_i)=\bfm_i(a_i)\) }\DIFaddend by stationarity of \DIFdelbegin \DIFdel{\(m_i\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i\)}\DIFaddend .
...
For two actions more is true: the two rows of \(K_i\) coincide\DIFdelbegin \DIFdel{, and the
marginal \(m_i\) takes }\DIFdelend \DIFaddbegin \DIFadd{. We call
this the }\emph{\DIFadd{forgetful}} \DIFadd{property, since the probability that a selected
player's next action is \(C\) is then the same whichever action they
currently hold, and it is what gives the marginal \(\bfm_i\) }\DIFaddend an explicit
closed form. \DIFdelbegin \DIFdel{For \(L_i\ge3\) the rowsdiffer in general, since the probability of retaining the current action
depends on that action, so the following }\DIFdelend \DIFaddbegin \DIFadd{The two switching probabilities are complementary, by
\(\phi_i(x)+\phi_i(-x)=1\), and with a single alternative this is enough to
align the rows. For \(L_i\ge3\) they always differ: identical rows would
force \(g_i\) to be constant, since \(\phi_i\) is injective, and equating a
diagonal entry with the off-diagonal entries in its column would then
require \(L_i=2\). The }\DIFaddend forgetful property is \DIFaddbegin \DIFadd{therefore }\DIFaddend special to two
actions.
...
by~\DIFdelbegin \DIFdel{\eqref{eq:intro-transition} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eq:kernel} }\DIFaddend the probability of switching to \(C\) when
selected is
\[
...
\DIFdelbegin \DIFdel{\(m_i=(p_i,\,1-p_i)\) }\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i=(p_i,\,1-p_i)\) }\DIFaddend is the unique stationary distribution of \(K_i\).
\DIFdelbegin \DIFdel{By
Theorem~\ref{thm:general} the stationary marginal of }\DIFdelend \DIFaddbegin \DIFadd{Additivity decouples }\DIFaddend player \(i\) \DIFdelbegin \DIFdel{'s action is \(m_i\), so the }\DIFdelend \DIFaddbegin \DIFadd{from the rest of the population: their
...
stationary distribution as \(K_i\). Hence the }\DIFaddend long-run probability that
player \(i\) cooperates is \DIFdelbegin \DIFdel{\(m_i(C)=p_i\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i(C)=p_i\)}\DIFaddend .

\end{proof}

\DIFdelbegin \DIFdel{Additivity makes }\DIFdelend \DIFaddbegin \DIFadd{Theorem~\ref{thm:decomp} gives the common value of the two rows.
Additivity and the restriction to two actions together make }\DIFaddend a player's
...
By the proof of Theorem~\ref{thm:decomp}, \DIFdelbegin \DIFdel{\(m_i=(p_i,\,1-p_i)\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i=(p_i,\,1-p_i)\)}\DIFaddend , so that
\DIFdelbegin \DIFdel{\(m_i(a_i)=p_i^{\,a_i}(1-p_i)^{1-a_i}\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i(a_i)=p_i^{\,a_i}(1-p_i)^{1-a_i}\)}\DIFaddend , and~\eqref{eq:stationary} is the
...
distribution \DIFdelbegin \DIFdel{\(m_i=(p_i,\,1-p_i)\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i=(p_i,\,1-p_i)\)}\DIFaddend , determined by the two-state balance
...
verbatim, and the stationary distribution is the product of the \DIFdelbegin \DIFdel{\(m_i\)}\DIFdelend \DIFaddbegin \DIFadd{\(\bfm_i\)}\DIFaddend .
...
\((j-1)\)-th. \DIFaddbegin \DIFadd{This geometric-sum reading requires the total contribution
...
\(w\to1\). }\DIFaddend For \(w>1\) each additional contribution is worth more than
...
correlations \(\mathrm{corr}(a_i,a_j)\)\DIFdelbegin \DIFdel{under the stationary distribution
for the }\DIFdelend \DIFaddbegin \DIFadd{, the Pearson correlation
...
for the }\DIFaddend heterogeneous group of Figure~\ref{fig:validation} (contributions
...
\begin{figure}[!htbp]
  ...
  \caption{Mutation in the non-linear public goods
    ...
    \textit{Bottom row:} pairwise \DIFaddbeginFL \DIFaddFL{Pearson }\DIFaddendFL correlations
    \(\mathrm{corr}(a_i, a_j)\) \DIFaddbeginFL \DIFaddFL{of the binary actions
    }\DIFaddendFL under the stationary distribution for the heterogeneous group of
...
\DIFaddbegin \DIFadd{Throughout, the mutation rates are exogenous: they model trembles,
...
\DIFaddend The source code, figures, and data for this paper are under version control
...
\DIFaddbegin \DIFadd{across two rounds of review}\DIFaddend , which led to substantial improvements
