...
difference. Couto and Pal
showed that for \emph{additive} games, those in which the payoff difference a
player evaluates when considering a switch is independent of the other
players' actions, the stationary distribution of introspection dynamics is a
product measure. We extend this result to introspection dynamics with
\emph{mutation}, where a selected player switches to cooperation or to
defection with payoff-independent, player-specific probabilities, and we
allow player-specific selection
intensities. We show that the product structure is preserved, and we obtain
an explicit formula for each player's long-run cooperation probability in
terms of their own payoffs, selection intensity, and mutation probabilities
...
\(2^N\) (more generally \(L^N\) when each player has \(L\) actions). They
identified an important exception: for \emph{additive} games, in which the
...
action set \(A_i\) with \(L_i:=|A_i|\ge2\). The state space is
\(S=\prod_{i=1}^{N}A_i\). Each player \(i\) has a payoff function
\(f_i:S\to\mathbb{R}\) and player-specific selection intensity \(\beta_i>0\).
For \(b\in A_i\) we write \(\bfa_{i\to b}\) for the state with player \(i\)'s
action replaced by \(b\), and
\[
...
\(\Delta f_i(\bfa;b)=g_i(a_i)-g_i(b)\). Indeed, fixing a reference action
\(c\in A_i\) and setting \(g_i(a_i):=\Delta f_i(\bfa;c)\) and
\(h_i(\bfa_{-i}):=f_i(\bfa_{i\to c})\) exhibits the decomposition, and the
converse is immediate.

Our running special case is that of two actions: \(A_i=\{C,D\}\) coded as
...
For \(M_1\) we have \(\delta_1=c_1\) and \(\delta_2=c_2\). However for \(M_2\),
...
More generally, the general Prisoner's Dilemma (PD) is parameterised by the
...
comparison. For two actions we write \(\mu_{i0}:=\mu_{iC}\) for the
probability of mutating to cooperation and \(\mu_{i1}:=\mu_{iD}\) for the
probability of mutating to defection, so that \(M_i=\mu_{i0}+\mu_{i1}\).
\end{itemize}

At each step, one player \(i\) is selected uniformly at random, and draws an
alternative action \(b\) uniformly from the \(L_i-1\) actions other than
\(a_i\). The transition probability for the switch is
...
\(\mu_{ib}=0\) recovers the introspection dynamics
...
\(\bar{a}_i=C\) and \(\mu_{i1}\) when \(\bar{a}_i=D\).
...
general result first and then specialise to two actions, where the marginals
take an explicit closed form.
...
\end{equation}
where \(m_i\) is the unique stationary distribution of \(K_i\).
...
\[
  ...
  = \pi_{\bfa},
\]
since \(\sum_{b} m_i(b)K_i(b,a_i)=m_i(a_i)\) by stationarity of \(m_i\).
...
by~\eqref{eq:intro-transition} the probability of switching to \(C\) when
selected is
\[
  K_i(D,C) = (1-\mu_{i0}-\mu_{i1})\phi_i(\delta_i)+\mu_{i0} = p_i.
\]
...
\(D\) is
\[
  K_i(C,D) = (1-\mu_{i0}-\mu_{i1})\bigl(1-\phi_i(\delta_i)\bigr)+\mu_{i1}
           = 1-p_i.
\]
...
Theorem~\ref{thm:general} the stationary marginal of player \(i\)'s action
...
selected they cooperate with the same probability \(p_i\), whatever their
current action. This probability is the payoff-driven Fermi value
...
By the proof of Theorem~\ref{thm:decomp}, \(m_i=(p_i,\,1-p_i)\), so that
\(m_i(a_i)=p_i^{\,a_i}(1-p_i)^{1-a_i}\), and~\eqref{eq:stationary} is the
...
For \(\mu_{i0}=\mu_{i1}=0\), Theorem~\ref{thm:stationary} is the two-action
...
For the Prisoner's Dilemma \(M_1\) of~\eqref{eq:games_from_couto_2022},
...
in~\cite{couto2022introspection}. For example, in the donation game \(M_1\)
...
measure with marginals \(\mu_{i0}\) or \(1-\mu_{i1}\). Mutation therefore
...
terms between players survive. For \(\mu_{i0}=\mu_{i1}=0\) and a single global
selection intensity, the product measure of Theorem~\ref{thm:stationary} reduces
...
\begin{figure}[!htbp]
  ...
  \caption{Validation of formula~\eqref{eq:pC} on a heterogeneous group with
    \(\alpha_i=i\), \(\beta=0.5\), \(\mu_{i0}=0.05\), \(\mu_{i1}=0.15\).
    ...
    Blue boxes: distribution of \(p_C\) across 19 independent runs of 5{,}000
    simulation steps each (with the first 500 steps discarded as warm-up;
    outliers shown).
...
\(\delta_i>0\)) a cooperative mutation bias \(\mu_{i0}>\mu_{i1}\) can still raise
...
\begin{figure}[!htbp]
  ...
  \caption{Structural properties of \(p_i\) for a single player with \(N = 5\)
    and, unless varied, symmetric mutation \(\mu_{i0} = \mu_{i1} = 0.1\).
    ...
    \(\mu_{i0} = \mu_{i1} = \mu\) (\(\alpha_i = 2\), \(\beta_i = 2\)) for
...
\begin{figure}[!htbp]
  ...
  \caption{Effect of mutation on the cooperation probability of a single player with
    ...
    \([\mu,1-\mu]\). Increasing the group size alone turns the net beneficiary into a
    net contributor.}
  \label{fig:mutation}
\end{figure}
...
does the opposite. Each player's long-run cooperation is thus determined entirely by their own
...
\begin{figure}[!htbp]
  ...
  \caption{Mutation in the non-linear public goods
    game~\eqref{eq:nonlinear-payoff} with \(N=8\) and \(\alpha=1\). All
    ...
    (\(w=0.7\)), the linear game (\(w=1\)), and synergy (\(w=1.4\)). Orange
    dots: the closed form of Corollary~\ref{cor:exact-pC}, which the
    \(w=1\) values match. The \(w=0.7\) curve follows the additive pattern
    ...
    \textit{Bottom row:} pairwise correlations \(\mathrm{corr}(a_i, a_j)\)
    under the stationary distribution for the heterogeneous group of
    Figure~\ref{fig:validation} (contributions \(\alpha_i=i\),
    \(\beta=0.5\)), at \(r=4\) and \(\mu=0.05\); diagonals are omitted and
    the colour scales differ between panels. For \(w=1\) every entry is
...
solvable: the stationary distribution is a product measure over the players
for any finite number of actions per player (Theorem~\ref{thm:general}),
reducing its computation to one small linear system per player. The
product-measure structure, due to~\cite{couto2023multiplayer} for the
no-mutation case, is thus preserved under mutation. For two actions, each
selection of player \(i\) results in action \(C\) with probability \(p_i\)
regardless of the current state (Theorem~\ref{thm:decomp}), and the marginals
take a closed form that makes the stationary distribution fully explicit
(Theorem~\ref{thm:stationary}), although this closed form is special to two
actions. For the heterogeneous public
...
The exact analysis does not extend to games that are not additive. When
...
system must be solved in general. Games such as the Stag Hunt (\(M_2\)
of~\eqref{eq:games_from_couto_2022}) contain payoff cross
terms \(a_i a_{-i}\) that cannot be eliminated by rescaling, so no reformulation
brings them within the scope of the closed forms. Our numerical exploration of
the non-linear public goods game (Section~\ref{sec:non-additive}) shows that
mutation then takes on a further role, selecting among coexisting strict
equilibria; characterising this behaviour analytically remains open.
