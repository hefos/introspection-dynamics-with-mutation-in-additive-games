...
Introspection dynamics is closely related to the logit-response dynamics of
evolutionary game theory in
economics~\cite{blume1993statistical,alosferrer2010logit}, in which revising
...
rule. Mutation, a random action chosen independently of payoffs, is a
standard ingredient of evolutionary and learning
...
From this extended dynamic we rederive, and generalise, the additive-game
...
\begin{equation}\label{eq:intro-transition}
  ...
  = \frac{1}{N}\bigl[(1-\mu_{iC}-\mu_{iD})\phi_i(\Delta f_i(\bfa))
    +\mu_{i,\bar{a}_i}\bigr],
\end{equation}
where the mutation term \(\mu_{i,\bar{a}_i}\) is indexed by the target
action \(\bar{a}_i\).
...
For \(\mu_{iC}=\mu_{iD}=0\), Theorem~\ref{thm:stationary} is the two-action
...
\(\nu_{i,\bar{a}_i}\in(0,1)\), and otherwise apply the Fermi comparison, so
that the switching probability of~\eqref{eq:intro-transition} becomes
...
mutation is read as an error in executing a switch. For more than two
actions, mutating to a uniformly chosen \emph{other} action still yields a
selection kernel that depends only on the player's own action, so
Theorem~\ref{thm:general} continues to apply there too.
...
terms between players survive. For \(\mu_{iC}=\mu_{iD}=0\) and a single global
