...
regardless of their current action. The chain is thus a random-scan Gibbs sampler
targeting \(\pi = \bigotimes_i \pi_i\): at each step one coordinate is resampled
from its marginal under \(\pi\), independently of all other coordinates. By the
standard theory of random-scan Gibbs samplers~\cite{liu1995covariance}, \(\pi\) is
stationary, and since \(T^{\intro}\) is ergodic (by \(\mu>0\)), it is the unique
