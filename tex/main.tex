...
For two actions more is true: the two rows of \(K_i\) coincide. We call
this the \emph{forgetful} property, since the probability that a selected
player's next action is \(C\) is then the same whichever action they
currently hold, and it is what gives the marginal \(\bfm_i\) an explicit
closed form. The two switching probabilities are complementary, by
...
require \(L_i=2\). The forgetful property is therefore special to two
actions.
...
Theorem~\ref{thm:decomp} gives the common value of the two rows.
...
across two rounds of review, which led to substantial improvements
